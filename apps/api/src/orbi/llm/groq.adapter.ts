...
import { createGroqClient, GROQ_DEFAULT_MODEL } from './groq-client';
import type { LlmAdapter, LlmEvent, LlmMessage, LlmToolDefinition } from './llm-adapter.interface';

// GroqAdapter es el FALLBACK de GeminiAdapter (ver FallbackLlmAdapter). Mismo
// contrato LlmAdapter. Ignora el `model?` que le pasa el controller —ese es un
// ID de Gemini por superficie— y usa su propio modelo (GROQ_MODEL o el default).
...
const MODELO_POR_DEFECTO = GROQ_DEFAULT_MODEL;
...
export class GroqAdapter implements LlmAdapter {
  ...
  get modelo(): string {
    return this.config.get<string>('GROQ_MODEL') ?? MODELO_POR_DEFECTO;
  }
  ...
  private getClient(): Groq {
    if (!this.client) {
      this.client = createGroqClient(this.config);
    }
    return this.client;
  }
