...
const HALLAZGOS: SeedItem[] = [
  ...
  hallazgo('auth-estado-en-memoria', G_INT, 'BAJA', 'Throttler y store de Google OAuth en memoria',
    'ThrottlerModule no tiene storage compartido y el canje de Google OAuth vive en un Map: con más de una instancia de Cloud Run los límites se cuentan por instancia y un canje puede caer en otra instancia y fallar. Hoy corre con una instancia, así que no duele. 15/09: deploy.sh activa --session-affinity (documentado en DEPLOYMENT.md): las requests de un mismo cliente van a la misma instancia, así el canje y los contadores no se pierden al escalar. Sin Redis a propósito: no hay tráfico que lo justifique.',
