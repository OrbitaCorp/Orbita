...
const HALLAZGOS: SeedItem[] = [
  ...
  hallazgo('drift-schema', G_AUD, 'ALTA', 'Drift del schema: wizard_ai_turns',
    'En producción existen model, prompt_tokens y completion_tokens con datos, no declaradas en schema.prisma. prisma migrate diff/dev genera un DROP de las tres. 15/09: verificado con prisma migrate diff contra producción: el schema declara model, prompt_tokens y completion_tokens y no quedan diferencias aparte de la migración del CUIT.',
  ...
  hallazgo('gasto-ia', G_AUD, 'MEDIA', 'Sin tope de gasto de IA (Groq/Gemini)',
    'El freno de USD 25 es de GCP y no cubre la IA. El 10/09 (auditoría interna, ítem api.orbi) se agregaron en código los topes de Orbi: 300 mensajes por negocio y 100 por IP del wizard por día, historial de 30 mensajes al modelo, conversación guardada de 200 y 6 vueltas de herramientas por mensaje; y en quitar fondo, 30 cada 10 minutos por negocio. Falta configurar alertas o topes de presupuesto en las consolas de Google AI Studio y Groq.',
  ...
  hallazgo('auth-estado-en-memoria', G_INT, 'BAJA', 'Throttler y store de Google OAuth en memoria',
    'ThrottlerModule no tiene storage compartido y el canje de Google OAuth vive en un Map: con más de una instancia de Cloud Run los límites se cuentan por instancia y un canje puede caer en otra instancia y fallar. Con min 0 y max 10 instancias, bajo carga puede haber más de una. 15/09: se evaluó la afinidad de sesión de Cloud Run y NO sirve acá: es por cookie y best effort, el canje lo crea el navegador y lo consume el BFF de Vercel server a server sin cookie, Firebase Hosting descarta las cookies que no sean __session y el navegador llama a la API sin credentials. No se activó. Sigue abierto: la solución real es el canje de Google en Postgres (fila con TTL de 60 s y consumo atómico) y, si algún día escala, el throttler con storage compartido o max-instances=1.',
  ...
  hallazgo('businesses-sin-baja', G_INT, 'BAJA', 'No existe la baja de un negocio',
    'No hay endpoint ni proceso para dar de baja un negocio: CANCELLED está en el enum de la suscripción pero no lo pone nadie, y el controller dice que DELETE /business quedó "fuera de esta fase". Un dueño que se va no tiene cómo pedir que se borren sus datos ni los de sus clientes (derecho de supresión, Ley 25.326). 15/09: la baja existe desde el ciclo de vida de suscripciones (cancelBusiness + ventana de 60 días con aviso a los 7, soft delete con deletedAt y borrado de refresh tokens), PERO NO pausa ni cancela la preapproval de Mercado Pago: un negocio con plan activo que se da de baja sigue siendo debitado, y el primer cobro aprobado (recordPayment) vuelve la suscripción a ACTIVE y le manda el mail de reactivación aunque la tienda siga dada de baja. Urgente, en el código de Mateo: pausar la preapproval al dar de baja, reanudarla al reactivar, cancelarla al cumplirse los 60 días, y que recordPayment no reactive una suscripción CANCELLED (registrar el cobro para reembolsarlo). Después, la supresión de datos personales a pedido, definiendo antes qué se conserva por obligación fiscal.',
  ...
  hallazgo('dominios-comprados-sin-renovacion', G_INT, 'BAJA', 'Dominios comprados: sin renovación ni transferencia',
    'Los dominios que se compran desde el panel quedan registrados en la cuenta de Vercel de OrbitaCorp con autoRenew apagado a propósito (no hay forma de recobrar la renovación, que para varios TLD cuesta 20 veces el primer año). Nadie avisa antes del vencimiento, y "Eliminar" en el panel lo saca del proyecto pero no se lo transfiere al dueño, que es el titular en el WHOIS. 15/09: el aviso ya está (DomainExpiryService en el mantenimiento nocturno: mail a cada owner a los 30 y a los 7 días, reintenta los envíos que fallan, no cambia el estado del dominio). Decidido: la renovación es manual a pedido del dueño (el mail lo dice) y se cobra aparte al precio del registrador; después de renovar en Vercel, soporte corre scripts/dominios/registrar-renovacion.cjs para correr expires_at. La transferencia, a pedido, por soporte.',
  ...
  hallazgo('rls-api-rest-abierta', G_INT, 'ALTA', 'La API REST de Supabase da acceso completo a la base con la clave anon',
    ...
    ['Migración 20260910110000_rls_tablas_publicas aplicada en producción', 'Verificado: 68 tablas con RLS y sin permisos para anon/authenticated', 'Clave anon rotada en el panel de Supabase', 'Decidido si se desactiva la Data API'],
    { ruta: 'prisma/migrations/20260910110000_rls_tablas_publicas · test/unit/rls-supabase.unit-spec.ts' }),
  hallazgo('rls-tabla-ciclo-vida', G_INT, 'MEDIA', 'La tabla del ciclo de vida de suscripciones quedó sin RLS',
    'La migración 20260910182701_subscription_lifecycle_fase0 (ciclo de vida de suscripciones) creó subscription_lifecycle_notices sin habilitarle RLS y se aplicó en producción el 10/09: es la única de las 69 tablas de public que quedó afuera del cierre de 20260910110000_rls_tablas_publicas. Atenuante: no tiene permisos para anon ni authenticated —el ALTER DEFAULT PRIVILEGES de aquella migración se los saca de antemano a toda tabla nueva—, así que hoy la clave anon no la alcanza; lo que se perdió es la defensa en profundidad, no el acceso. Detectado el 12/09 al mergear la rama de la auditoría, por el tercer test de rls-supabase.unit-spec.ts. Decisión del 12/09: dejarlo abierto sin tocar producción, y que el test lo tolere con una excepción documentada.',
    ['ALTER TABLE subscription_lifecycle_notices ENABLE ROW LEVEL SECURITY aplicado en producción', 'La tabla sale de EXCEPCIONES en rls-supabase.unit-spec.ts'],
    { ruta: 'prisma/migrations/20260910182701_subscription_lifecycle_fase0 · test/unit/rls-supabase.unit-spec.ts' }),
  hallazgo('auditoria-sin-pantalla', G_INT, 'BAJA', 'El registro de auditoría no tiene pantalla en el panel',
    'Desde el 10/09 las acciones sensibles del panel quedan en audit_logs y GET /audit-logs responde con el permiso config.audit.view, pero el dueño todavía no tiene dónde verlo. Además quedan por enganchar descuentos, cupones, credenciales de Mercado Pago y dominios. 15/09: lo de enganchar ya está (auditoria-acciones-sin-registro cerrado en tres partes); la pantalla del panel va al backlog de producto.',
  ...
  hallazgo('mensajes-cliente-sin-throttle', G_INT, 'BAJA', 'El chat de la tienda no tenía throttle ni tope de mensajes',
    'Un cliente logueado podía mandarle mensajes a la tienda sin límite propio (llenar la bandeja), y abrir un hilo devolvía todos sus mensajes sin tope. Cerrado el 10/09 en la auditoría del ítem api.conversations: 20 mensajes por minuto y los últimos 500 al abrir.',
    ['Throttle al mensaje del cliente', 'Últimos 500 mensajes al abrir un hilo', 'Tests unitarios', 'Desplegado'],
    { estado: 'HECHO', checksHechos: true, ruta: 'me-conversation.controller.ts · conversations.service.ts' }),
  hallazgo('notificacion-mail-sin-escapar', G_INT, 'MEDIA', 'El mail de los avisos al dueño metía el nombre del cliente como HTML',
    'Los avisos "nuevo pedido" y "nuevo cliente" llevan el nombre que el cliente escribe al registrarse, y el mail a los miembros lo metía crudo en el HTML: un "<a href=...>" ahí salía como link real, con la marca de la propia tienda. Cerrado el 10/09 en la auditoría de api.mail (se escapa el cuerpo), junto con asuntos de una sola línea y un solo destinatario por envío en todo MailService.',
    ['Cuerpo del aviso escapado', 'Asuntos sin saltos de línea y destinatario único en MailService', 'Tests unitarios', 'Desplegado'],
    { estado: 'HECHO', checksHechos: true, ruta: 'notifications.service.ts#sendEmailToMembers · mail.service.ts' }),
  hallazgo('alta-directa-sin-pago', G_INT, 'ALTA', 'Se podían crear negocios sin pagar y ocupar el email de otra persona',
    'POST /onboarding/register-business (público) creaba cuenta y negocio sin pasar por el pago y con el email marcado como verificado. No se podía publicar sin suscripción, pero sí llenar la base de negocios y registrar un email ajeno, que después no podía darse de alta. El alta real ya pasaba por /subscription/checkout + confirmAndCreate. Cerrado el 10/09 en la auditoría de api.onboarding: en producción responde 404 (salvo PERMITIR_REGISTRO_DIRECTO=true) y tiene throttle.',
    ['register-business apagado en producción', 'Throttle en register-business, check-email y check-subdomain', 'Tests unitarios', 'Desplegado'],
    { estado: 'HECHO', checksHechos: true, ruta: 'onboarding.controller.ts#register' }),
  hallazgo('inventario-mock-sin-montar', G_INT, 'BAJA', 'Prototipo de Inventario con datos de ejemplo en el repo',
    'La carpeta apps/web/src/modules/ventas/panel/inventario (stock general, entrada, ajuste, movimientos, proveedores) es un prototipo con datos de un mock: el ajuste no llama a la API, solo muestra "Ajuste registrado". No está en el componentMap de AdminSeccionShell, así que nadie lo abre, pero confunde. Reportes → Inventario mostraba ese mismo mock como si fueran los números del negocio: desde el 10/09 (ítem web.panel.reportes) dice que el reporte no está disponible y manda a Productos. Falta el reporte real, calculado en el servidor, y borrar el prototipo y reportes/mock/reportes.mock.ts (sin uso). Detectado el 10/09 en la auditoría de web.panel.inventario.',
    ['Borrar la carpeta panel/inventario y reportes/mock/reportes.mock.ts', 'Reporte de inventario valorizado real, calculado en el servidor (definir si se valoriza a precio o a costo)', 'Reportes → Inventario deja de mostrar datos de ejemplo (hecho el 10/09)'],
    { ruta: 'apps/web/src/modules/ventas/panel/inventario · reportes/ReporteInventario.tsx' }),
  hallazgo('csp-scripts', G_INT, 'BAJA', 'La web no tiene una Content-Security-Policy de scripts',
    'Desde el 10/09 (ítem web.middleware) next.config manda HSTS, nosniff, X-Frame-Options SAMEORIGIN, Referrer-Policy y una CSP mínima (frame-ancestors, base-uri, object-src). Falta la parte que más protege ante un XSS: script-src/connect-src. Hay que armarla con nonces para el script inline de tema de _app.tsx y los de Next, permitir Google Fonts, la API y los retornos de Mercado Pago, probarla primero en Content-Security-Policy-Report-Only y recién después aplicarla. No se hizo sin supervisión porque una CSP mal armada deja la tienda en blanco.',
    ['CSP en modo Report-Only con script-src y connect-src', 'Nonce para el script inline de tema (_app.tsx)', 'Revisar los reportes una semana y pasarla a CSP efectiva'],
    { ruta: 'apps/web/next.config.ts · apps/web/src/pages/_app.tsx' }),
  hallazgo('legales-sin-cuit', G_INT, 'MEDIA', 'Los Términos y la Política de privacidad de cada tienda no muestran el CUIT del Comercio',
    'Las plantillas de docs/legales piden "{{nombre_comercio}}, CUIT {{cuit_comercio}}", pero el negocio no tiene un campo de CUIT (ni de razón social): lib/storefront/legales.ts omite la frase. La identificación del vendedor es un dato que la normativa de comercio electrónico espera ver en la tienda. Hace falta el dato: migración + campo en Configuración + exponerlo en la config pública. Detectado el 10/09 en la auditoría de web.cliente.legales. 15/09: hecho en código (9e990): CUIT y razón social en Configuración → Contacto, con dígito verificador, y en Términos y Privacidad. Falta revisar el texto con un abogado.',
    ['Campo CUIT (y razón social) en BusinessConfig, con migración', 'Se carga en Configuración → General y se valida (11 dígitos, dígito verificador)', 'legales.ts lo completa en Términos y Privacidad', 'Revisar el texto con un abogado (lo pide la propia plantilla)'],
    { ruta: 'apps/web/src/lib/storefront/legales.ts · prisma/schema.prisma (BusinessConfig)' }),
  hallazgo('alta-sin-verificar-email', G_INT, 'BAJA', 'El alta paga no verifica que el email sea de quien paga',
    'Al confirmarse el pago, la cuenta se crea con emailVerified: true sin haber mandado ningún código al email. Quien paga podría registrar un email ajeno y dejar a esa persona sin poder darse de alta (el email es único entre dueños). El costo del pago lo vuelve improbable, pero conviene un código de verificación antes del checkout. Detectado el 10/09 en la auditoría de api.onboarding. 15/09: postergado a propósito: agregar un código al wizard es fricción en el alta paga y el riesgo es teórico (hay que pagar para bloquear un mail ajeno). Se retoma junto con la confirmación de email de members, con un solo flujo.',
    ['Código de verificación al email antes de /subscription/checkout', 'emailVerified solo en true después del código', 'Tests'],
    { ruta: 'subscriptions.service.ts#startCheckoutPending · onboarding.service.ts#registerBusiness' }),
  hallazgo('imagenes-sin-tope-de-pixeles', G_INT, 'MEDIA', 'Las imágenes subidas se decodificaban sin tope de píxeles',
    'Las subidas tienen un tope de 10 MB, pero un PNG de 10 MB puede declarar cientos de megapíxeles y sharp los decodificaba enteros (su tope por defecto es 268 MP): alcanzaba para tirar la memoria de la instancia de Cloud Run. Pasaba en todas las subidas, incluido el avatar del cliente, que la puede subir cualquiera que se registre en una tienda. Además, quitar el fondo corría el modelo en la CPU de la API sin límite por negocio. Cerrado el 10/09 en la auditoría del ítem api.background-removal.',
    ['Tope de 60 MP al decodificar en todas las subidas (logo, Apariencia, productos, avatar, quitar fondo)', 'Quitar fondo: 30 cada 10 minutos por negocio y 2 a la vez', 'Errores claros (400/429)', 'Tests unitarios', 'Desplegado'],
    { estado: 'HECHO', checksHechos: true, ruta: 'common/utils/subida-imagen.ts · background-removal.service.ts · businesses.service.ts · products.service.ts · me.service.ts' }),
  hallazgo('juegos-premios-ilimitados', G_INT, 'ALTA', 'Un cliente podía cobrar premios de los juegos sin límite',
    'El "un premio por campaña" de los juegos con premio vivía solo en el localStorage del navegador: con sesión, un cliente jugaba, ganaba, cobraba y repetía, y cada reclamo creaba un cupón de descuento real de hasta el techo del juego (que el dueño puede poner en 100%). Además, dos "terminar" o dos "reclamar" simultáneos de la misma sesión creaban dos cupones. En producción al 10/09: 11 cupones de premio, 9 de un mismo cliente de prueba, ninguno usado. Cerrado el 10/09 en la auditoría del ítem api.games.',
    ['Un premio por cliente, juego y campaña, verificado en el servidor', 'Terminar y reclamar con escritura condicionada y lock por cliente', 'Tests unitarios', 'Desplegado'],
    { estado: 'HECHO', checksHechos: true, ruta: 'games-play.service.ts#yaGanoEnLaCampania · #finishSession · #claimInternal' }),
  hallazgo('prueba-social-id-pedido', G_INT, 'MEDIA', 'El feed público de prueba social exponía el id real de los pedidos',
    'GET /storefront/:slug/social-proof/recent devolvía, en cada aviso, el id del pedido. Para una compra como invitado, conocer ese id es la única prueba de pertenencia que piden los flujos de pago (armar la preferencia de Mercado Pago, sincronizar el pago al volver): publicarlo en un endpoint abierto la regalaba. Al 10/09 ningún negocio tenía la prueba social configurada. Cerrado el 10/09 en la auditoría del ítem api.social-proof: identificador opaco, y sin avisos en tiendas pausadas o sin publicar.',
    ['Identificador opaco en vez del id del pedido', 'Sin avisos en tiendas pausadas o sin publicar', 'Tests unitarios', 'Desplegado'],
    { estado: 'HECHO', checksHechos: true, ruta: 'social-proof.service.ts#getRecentEvents · storefront-social-proof.controller.ts' }),
  hallazgo('promo-modal-link-y-vigencia', G_INT, 'BAJA', 'El botón del modal de anuncios podía llevar a cualquier sitio',
    'ctaLink aceptaba cualquier texto y la tienda, si empezaba con http(s)://, hacía window.location.href: desde la marca de la tienda se podía mandar al cliente a cualquier sitio (hallazgo bajo del 04/09). Además la vigencia de solo día terminaba a las 21 h del día anterior, y el endpoint público servía el modal de tiendas pausadas o no publicadas. Cerrado el 10/09 en la auditoría del ítem api.promo-modal.',
    ['ctaLink solo como ruta de la tienda (y los guardados externos se descartan)', 'Vigencia por días de Argentina', 'Sin modal en tiendas pausadas o no publicadas', 'Tests unitarios', 'Desplegado'],
    { estado: 'HECHO', checksHechos: true, ruta: 'dto/upsert-promo-modal.dto.ts · storefront-promo-modal.controller.ts · promo-modal.service.ts · PromoModalConfig.tsx' }),
  ...
  hallazgo('resenas-sin-moderacion-panel', G_INT, 'BAJA', 'El negocio no puede ver ni moderar sus reseñas desde el panel',
    'Existe PATCH /reviews/:id/hide (owner/admin), pero ningún endpoint lista las reseñas del negocio (incluidas las ocultas), no hay forma de volver a mostrar una ocultada y el panel no tiene pantalla: el dueño no se entera de lo que le escriben ni puede sacar una reseña ofensiva. Hoy solo el super admin ve las reseñas de un negocio. 15/09: es una feature de producto, no un arreglo: va al backlog como pantalla de Reseñas del panel. Mientras tanto no hay forma soportada de ocultarla desde soporte: el super admin solo ve las reseñas y PATCH /reviews/:id/hide es del owner o admin del negocio, sin pantalla. Hoy sería un update manual en la base.',
  ...
  hallazgo('cambio-plan-activo-no-se-aplica', G_INT, 'MEDIA', 'Cambiar de plan con un plan activo nunca se aplica',
    'Con un plan ya activo, el cambio queda en nextPlan "para la próxima renovación". Pero cada cobro de la preapproval vieja renueva el período, así que el período nunca termina y activatePlan() (que exige el período vencido) no se puede usar: el negocio sigue pagando el plan anterior para siempre, y el panel le dice que el cambio está programado. MP no deja cambiarle la frecuencia a una preapproval autorizada. En producción, 0 suscripciones con nextPlan al 10/09. 15/09: queda con Mateo, que está rehaciendo los planes (semestral y anual, 1cf46) y es quien toca changePlan; no se mete otra mano en paralelo.',
  ...
  // Transversales (auditoría interna 10/09, sección Transversal).
  hallazgo('rate-limit-ip-proxy', G_INT, 'ALTA', 'Los límites por IP ven la IP del proxy, no la del cliente',
    'main.ts no configura trust proxy: detrás de Firebase Hosting y Cloud Run, req.ip es la del proxy, así que todos los @Throttle (login, olvidé mi contraseña, soporte, validar cupones, juegos) y el tope de 100 mensajes por IP del wizard de Orbi los comparten todos los usuarios que entran por ese proxy: uno solo que abuse bloquea el login o Orbi para todos. Además el BFF de la web no reenvía la IP del cliente, así que login, refresh y registro llegan todos desde Vercel. Queda preparado y apagado: TRUST_PROXY_HOPS (el número exacto de saltos, nunca true) y GET /api/v1/health/ip para medirlo. Detectado el 10/09 en la auditoría de trans.rate-limit.',
    ['Medir los saltos con GET /api/v1/health/ip (comparando con la IP real) y cargar TRUST_PROXY_HOPS en env-vars.yaml', 'El BFF manda la IP del cliente con un secreto compartido, y un getTracker propio la usa solo si el secreto coincide', 'Probado: dos IPs distintas no comparten balde', 'Desplegado'],
    { ruta: 'main.ts · common/utils/proxy.ts · apps/web/src/lib/auth/bff.ts' }),
  hallazgo('mp-plan-override-prod', G_INT, 'ALTA', 'El beneficio de bienvenida cobra $15 cada 3 días en producción',
    'deploy/env-vars.yaml tenía MP_PLAN_OVERRIDE: "true" con MP_SUBSCRIPTION_AMOUNT 15 cada 3 días. El propio comentario del archivo dice que es para probar el checkout real y que hay que apagarlo al terminar: con eso prendido, cualquier alta real paga $15 en vez del precio de bienvenida, y ya pasó una vez. Detectado el 10/09 en la auditoría de trans.secretos. 15/09: Ale decidió dejarlo en true mientras Mateo lo usa para pruebas. 16/09: encontrado todavía prendido (desde el 28/07 como mínimo) al revisar por qué un cupón de prueba mostraba $15 en el wizard — apagado ese mismo día, primero en el servicio de Cloud Run en vivo y después en env-vars.yaml. Revisadas las Subscription de toda la base: solo 2 negocios de prueba (creados 28/07 y 09/09, ambos con nombre obviamente de prueba) quedaron con el monto de $15 — ningún cliente real pagó de menos.',
    ['Confirmado si la prueba sigue', 'MP_PLAN_OVERRIDE en "false" (o sin la línea) y desplegado', 'Revisadas las altas que se cobraron con el monto de prueba'],
    { estado: 'HECHO', checksHechos: true, ruta: 'deploy/env-vars.yaml · subscriptions.service.ts#bienvenida' }),
  hallazgo('secretos-tarball-cloud-build', G_INT, 'ALTA', 'Una copia vieja del .env pudo subir en cada deploy al bucket de Cloud Build',
    'apps/api/.env.bak2 (24/07) tiene DATABASE_URL, SUPABASE_SERVICE_ROLE_KEY, JWT_SECRET, GOOGLE_CLIENT_SECRET y MAIL_PASS. No había .gcloudignore y el .gitignore de apps/api solo excluye .env, así que gcloud builds submit muy probablemente lo subió en el tarball de fuentes de cada deploy (gs://orbita-api-corp_cloudbuild/source/). No llega a la imagen. Desde el 10/09 hay un .gcloudignore que excluye .env y .env.*. Detectado el 10/09 en la auditoría de trans.secretos.',
    ['.gcloudignore que excluye .env y .env.*', 'Revisados los tarballs de gs://orbita-api-corp_cloudbuild/source/ y borrados los que tengan el archivo', 'Rotados los valores del archivo que sigan en uso', 'Borrado apps/api/.env.bak2', 'Procedimiento de rotación y registro de quién tiene el .env de producción en DEPLOYMENT.md'],
    { ruta: 'apps/api/.gcloudignore · deploy/deploy.sh' }),
  hallazgo('cors-dominios-propios', G_INT, 'MEDIA', 'Las tiendas con dominio propio no pasan el CORS de la API',
    'El CORS de la API acepta orbita.site y sus subdominios, pero no los dominios propios verificados. La tienda en un dominio propio llama a la API desde el navegador (lib/storefront/api.ts), así que carrito, checkout y juegos quedarían bloqueados en cuanto se active un dominio. Al 10/09 hay un solo dominio, vinculado y PENDING: no afecta a nadie todavía. Detectado el 10/09 en la auditoría de trans.cors-headers.',
    ['origin de CORS que acepta CustomDomain ACTIVE con dnsVerified, con caché de unos minutos', 'Test unitario', 'Probado con un dominio real antes de activar el primero', 'Desplegado'],
    { ruta: 'main.ts (enableCors) · domains/' }),
  hallazgo('auditoria-acciones-sin-registro', G_INT, 'MEDIA', 'Acciones sensibles que no quedan en audit_logs',
    'audit_logs (que se empieza a escribir con el deploy pendiente del 10/09) no registra: conectar o desconectar Mercado Pago, dominios, descuentos y cupones, cancelaciones y reembolsos, inventario, sucursales, categorías, etiquetas, ni editar o borrar clientes. En el super admin no quedan registrados el envío de mails de prueba ni el login y el segundo factor de los admins. Detectado el 10/09 en la auditoría de trans.logging.',
    ['AuditService en Mercado Pago, dominios, descuentos, cancelaciones y reembolsos', 'AuditService en inventario, sucursales, categorías, etiquetas y clientes', 'platform_admin_logs: login, segundo factor y mails de prueba'],
    { ruta: 'audit/audit.service.ts · platform/' }),
  hallazgo('logs-sin-retencion', G_INT, 'BAJA', 'Sin retención definida para logs y registros de auditoría',
    'Solo la analítica del wizard tiene borrado automático (180 días, pendiente de deploy). platform_admin_logs, audit_logs y email_logs crecen para siempre, y la retención de los logs de Cloud Run no está decidida ni documentada. Detectado el 10/09 en la auditoría de trans.logging.',
    ['Decidida la retención de cada tabla', 'Purga en el mantenimiento nocturno', 'Retención de Cloud Logging documentada en DEPLOYMENT.md']),
  hallazgo('backups-sin-verificar', G_INT, 'ALTA', 'Backups de la base sin verificar ni probar',
    'El repo no documenta backups: ni PITR, ni pg_dump, ni cómo restaurar, ni RPO/RTO. Depende de lo que incluya el plan de Supabase, y los backups de la base probablemente no incluyen los archivos de Storage (logos y fotos). Detectado el 10/09 en la auditoría de trans.backups.',
    ['Confirmado en el dashboard qué backups hay y cada cuánto corren', 'Restauración de prueba en un proyecto aparte (nunca sobre producción)', 'RPO/RTO decididos y documentados en DEPLOYMENT.md', 'Copia de MERCADOPAGO_TOKEN_KEY y JWT_SECRET fuera de Cloud Run']),
  hallazgo('e2e-aislamiento-por-modulo', G_INT, 'MEDIA', 'Falta una prueba de negocio A contra negocio B por módulo',
    'auth-isolation.e2e cubre el login cruzado, pero productos no tiene ninguna prueba entre negocios, y pedidos, clientes y conversaciones solo tienen unit specs con Prisma simulado (sus e2e prueban dos clientes del mismo negocio). El barrido estático aislamiento-consultas.unit-spec.ts complementa pero no reemplaza. Depende de la base de prueba (hallazgo base-prueba). Detectado el 10/09 en la auditoría de trans.multi-tenant-e2e.',
    ['tenant-isolation.e2e parametrizado: con el token de B, 404 en los recursos de A (productos, pedidos, clientes, descuentos, conversaciones)', 'Corre contra la base de prueba', 'Job de e2e en CI'],
    { ruta: 'test/auth-isolation.e2e-spec.ts · test/unit/aislamiento-consultas.unit-spec.ts' }),
  hallazgo('rollback-sin-simulacro', G_INT, 'BAJA', 'El rollback de la API nunca se probó',
    'DEPLOYMENT.md explica cómo volver a una revisión anterior, pero no hay constancia de que se haya probado, falta la variante de mover el tráfico sin volver a desplegar y no advierte qué pasa con una migración destructiva. Detectado el 10/09 en la auditoría de trans.deploy-manual.',
    ['Simulacro de rollback con gcloud run services update-traffic', 'DEPLOYMENT.md con el rollback por tráfico y el aviso sobre migraciones destructivas'],
    { ruta: 'DEPLOYMENT.md § Rollback' }),
];
