...
// guardar/restaurar el scroll. Con una sola página no era un problema, pero
// apenas "Sobre nosotros" se separó del home a su propia ruta, mantener dos
// copias de ~200 líneas de CSS y del bug ya resuelto del scroll-restore
// (ver comentario más abajo) era pedir que una de las dos copias se
// desactualizara. Ahora ambas páginas envuelven su contenido en <PaginaV2>.
...
export function PaginaV2({ children, scrollKey }: { children: ReactNode; scrollKey: string }) {
    ...
    // forceDark: se sacó el modo claro de estas páginas — el diseño está
    // pensado en oscuro (fondo espacial, planeta) y el claro quedaba como una
    // versión de segunda, no una alternativa real.
    return (
        <ThemeProvider forceDark>
            <div className="oc-page relative min-h-screen" style={{ background: 'var(--oc-bg)' }}>
                <style>{`
                    /* ── Paleta ───────────────────────────────────────────────
                       Todas las páginas del rediseño pintan con estas variables,
                       así el tema claro es un solo bloque de overrides y no una
                       cacería de colores por cada componente. Los valores claros
                       se eligieron para pasar 4.5:1 sobre el fondo (texto
                       principal #0f172a y secundario #475569 sobre #f8fafc). */
                    .oc-page {
                        --oc-bg:            #000;
                        --oc-text:          #ffffff;
                    ...
                    /* Clases de texto de Tailwind usadas en el rediseño, mapeadas a
